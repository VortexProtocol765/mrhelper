...
function initLayerControl() {
    const layerControl = L.control.layers(mapLayers, null, {
        position: 'topright',
        collapsed: true
    }).addTo(map);
    
    // Add icons to layer labels
    setTimeout(() => {
        const labels = document.querySelectorAll('.leaflet-control-layers-base label');
        labels.forEach(label => {
            const span = label.querySelector('span');
            if (span) {
                const mapType = span.textContent.trim();
                let icon;
                switch(mapType) {
                    case 'Satellite':
                        icon = '<i class="fas fa-satellite" style="color: #4CAF50; margin-right: 8px;"></i>';
                        break;
                    case 'Street':
                        icon = '<i class="fas fa-map" style="color: #4CAF50; margin-right: 8px;"></i>';
                        break;
                    case 'Topographic':
                        icon = '<i class="fas fa-mountain" style="color: #4CAF50; margin-right: 8px;"></i>';
                        break;
                    case 'Dark':
                        icon = '<i class="fas fa-moon" style="color: #4CAF50; margin-right: 8px;"></i>';
                        break;
                }
